...
const browserChannelEl = document.getElementById("browserChannel");
...
startBtn.addEventListener("click", async () => {
  ...
  const browserChannel = browserChannelEl.value;
...
continueBtn.addEventListener("click", async () => {
  const result = await window.ebLauncher.continueScript();
  statusEl.textContent = result.message;
  if (result.ok) {
    continueBtn.disabled = true;
    appendLog("\n▶️ 已继续执行\n");
...
window.ebLauncher.onPaused(({ message }) => {
  statusEl.textContent = "已暂停，等待手动处理后继续";
